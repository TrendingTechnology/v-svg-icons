<template>
  <div class="container">
  <div class="alert" v-if="filteredList.length == 0">No results found for <b>{{ searchText }}</b>!</div>
  ...
  computed: {
    filteredList(){
      return this.$store.state.iconList.filter(icon => {
        return icon.toLowerCase().includes(this.searchText.trim().toLowerCase())
      })
    }
  }
}
...
.container{
  ...
  .icon-box{
    ...
    .icon{
      ...
      &.active{
        background: #CCE5FF;
        box-shadow: 2px 5px 5px rgba(black, .2);
        &:hover{
          background: #CCE5FF;
        }
      }
      *::selection{
      ...
      span{
        margin-top: 15px;
        font-size: 14px;
        font-family: sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol";
        color: rgba(0, 0, 0, .4);
      }
    }
  }
